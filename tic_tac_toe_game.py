...
players = ['игрок 1', 'игрок 2']  # имена игроков
n = 3  # размер поля 3 x 3 по умолчанию
spare_squares = [1, 2, 3, 4, 5, 6, 7, 8, 9]  # свободные квадраты для n = 3
symbols = spare_squares[:]  # символы для отображения на игровом поле, в начале игры все квадраты свободны
# count = 0  # счётчик ходов игроков
p1_moves = set()  # множество ходов I игрока
p2_moves = set()  # множество ходов II игрока
signs = ['- X -', '- O -']  # список символов "крестик" и "нолик"
end = 'Игра окончена.'  # заголовок окончания игры


def wincomb(n: int) -> list[set]:
    """
    Создаёт и возвращает список выигрышных комбинаций -
    множеств, состоящих из номеров квадратов игрового поля:
    рядов, столбцов, 2-х диагоналей.

    :param n: размер поля n x n
    :return: отображение списка множеств выигрышных комбинаций
    """
...
def validation(player_moves: set) -> bool:
    """
...
def congratulation(player: str) -> None:
    """
    Выводит на печать поздравление игроку-победителю.
...
def request(player: str, sign: str) -> None:
    """
...
def player_move(player: str, sign: str, player_moves: set, symbols: list, spare_squares: list) -> None:
    """
...
def preface(players: list[str], signs: list[str]) -> tuple[str, list, list]:
    ...
    sleep(1)
    print()
    display.print_paying_field(3, spare_squares)
    sleep(2)
    print()
    ...
    while not all(check_size(n)):
        if not check_size(n)[0]:
            ...
            print('-----')
            print(f"Число {n} слишком малнькое, оно должно быть не меньше 3.")
            print('-----')
        n = input(
            f'{p2.capitalize()}, выберите размер игрового поля: минимальный размер 3 x 3. В ведите целое число не меньше 3.')
...
n, players, signs = preface(players, signs)
n = int(n)
p1, p2 = players
wincomb = wincomb(n)
...
def main() -> None:
    """
    Выводит на печать пустое игровое поле n x n.
    Запускает цикл до тех пор, пока не выявлен
    победитель или остаются ходы:
    Игрок I делает ход,
    Выводит на печать обновлённое игровое поле.
    Если счётчик ходов больше порогового значения -
    запускается проверка на выигрыш.
    Если I игрок выиграл - объявляет победителя,
    завершает игру.
    Игрок II делает ход,
    Выводит на печать обновлённое игровое поле.
    Если счётчик ходов больше порогового значения -
    запускается проверка на выигрыш.
    Если II игрок выиграл - объявляет победителя,
    завершает игру.
    Если победитель не выявлен и ходов не осталось -
    объявляет ничью, завершает игру.
